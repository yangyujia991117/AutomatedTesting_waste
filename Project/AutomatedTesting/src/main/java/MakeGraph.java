...
/**
 * 该类的analysis方法用于遍历所有类所有方法、找出调用依赖关系，draw方法调用Graph的去除冗余方法和makeDotFile方法生成.dot文件
 */
public class MakeGraph {

  //用于遍历所有类所有方法、找出调用依赖关系
        ...
        if ("Application".equals(method.getDeclaringClass().getClassLoader().toString())) {
          ...
          while (iterator.hasNext()) {
            ...
            if (!(node1classInnerName.startsWith("Ljava")||node1classInnerName.startsWith("Ljavax"))){
              ...
              methodGraph.addNodes(signature, node1signature);//把调用信息加进两个Graph里
            }
          }
        }
      }
  ...
  //调用Graph的去除冗余方法和makeDotFile方法生成最终的.dot文件
